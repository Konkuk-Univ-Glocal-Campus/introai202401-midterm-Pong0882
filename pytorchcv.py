...
## gpu 있으면 gpu 쓰고 없으면 cpu를 쓰겠다는 코드

# Python에서 MNIST 데이터셋을 불러와서 처리하는 과정
# 이 함수를 실행하면, builtins 모듈을 통해 전역 변수로 설정된 data_train, data_test, train_loader, test_loader가 생성되어 어디서든 접근할 수 있게 됩니다. 
# 이러한 설정은 함수 내에서 데이터를 처리하고, 이후에 다른 부분에서 해당 데이터를 사용할 때 유용하게 활용될 수 있음

def load_fashion_mnist(batch_size=64): # load_fashion_mnist라는 이름의 함수를 정의하고, 이 함수는 기본적으로 batch_size 매개변수를 64로 설정합니다. 
    ## 배치 사이즈가 작아지면 적은 메모리로 빨른 훈련속도를 제공 / 훈련과정 불안정해짐
    # 이 매개변수는 데이터를 얼마나 많은 단위로 나눌지 결정
    builtins.data_train = torchvision.datasets.FashionMNIST('./data', # torchvision 라이브러리의 datasets 모듈을 사용하여 FashionMNIST 데이터셋을 불러옵니다.
        download=True, train=True, transform=ToTensor())
    # './data'는 데이터셋이 저장될 경로를 지정하며, download=True는 해당 경로에 데이터가 없을 경우 인터넷에서 자동으로 다운로드하도록 설정합니다. 
    ...
    # 학습 데이터셋을 데이터 로더에 로드합니다. 
    builtins.test_loader = torch.utils.data.DataLoader(data_test, batch_size=batch_size)
    # 데이터 로더는 데이터셋을 지정된 배치 크기에 맞게 나누고, 이를 반복 가능한 객체로 만들어 학습 과정에서 쉽게 사용할 수 있게 도움
...
def train_epoch(net,dataloader,lr=0.01,optimizer=None,loss_fn = nn.NLLLoss()): # 이 함수는 여러 매개변수를 받는데, net은 학습할 신경망 모델, dataloader는 데이터 로더,
    # lr은 학습률(기본값 0.01), optimizer는 최적화 도구(기본값은 None), loss_fn은 손실 함수로 기본적으로 Negative Log Likelihood Loss를 사용
    ## 한번의 학습단계에서 가중치 업데이트를 할때 해당 값에 lr 수치만큼 곱하여 적용되는것
    ...
    for features,labels in dataloader: # 데이터 로더로부터 특징(feature)과 레이블(label)을 반복적으로 가져옴
        ...
        ## cpu와 gpu 둘다 같은위치에 정보가 있어야 하기때문에 이동시키는 작업
...
def plot_results(hist): # plot_results라는 함수를 정의하는데 hist라는 이름의 딕셔너리를 매개변수로 받는데 학습과 검증 과정의 정확도와 손실이 배열 형태로 저장되어 있음
    ...
    plt.plot(hist['val_acc'], label='Validation acc') # hist 딕셔너리에서 검증 정확도(val_acc)를 추출하여 그래프로 그리는데 라벨을 'Validation acc'로 지정
    ###### 벨리데이션 데이터 없음 >>> 수정?
    ## 사실 벨리데이션 데이터였는데 우리는 그걸 테스트데이터로 보고있었다고?
...
def train(model, train_loader, test_loader, epochs=5):
    # GPU 사용 가능 여부에 따라 장치를 선택합니다.
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # 모델을 선택한 장치로 이동합니다.
    model.to(device)
    # Adam 옵티마이저를 사용하여 모델의 파라미터를 최적화합니다.
    optimizer = optim.Adam(model.parameters(), lr=0.001)
    # 교차 엔트로피 손실 함수를 사용하여 손실을 계산합니다.
    criterion = torch.nn.CrossEntropyLoss()
    
    # 학습 및 테스트 과정에서의 손실과 정확도를 기록하기 위한 딕셔너리입니다.
    ...
    try:
        ...
        for epoch in range(epochs):
            ...
            # 학습 데이터 로더에서 데이터와 타겟을 순회합니다.
            for data, target in train_loader:
                # 데이터와 타겟을 선택한 장치로 이동합니다.
                ...
                # 모델 출력이 None인 경우 경고를 출력하고 건너뜁니다.
                if output is None:
                ...
                # 손실이 None인 경우 경고를 출력하고 건너뜁니다.
                if loss is None:
                    print("Warning: Loss computation returned None.")
                    continue
                
                # 역전파를 통해 그라디언트를 계산하고 모델의 파라미터를 업데이트합니다.
                loss.backward()
                optimizer.step()
            ...
            # 평가 과정에서는 그라디언트 계산이 필요 없으므로 no_grad로 감싸줍니다.
            with torch.no_grad():
                # 테스트 데이터 로더에서 데이터와 타겟을 순회합니다.
            ...
            # 평가 손실과 정확도를 계산하고 기록합니다.
            test_loss /= len(test_loader.dataset)
            test_acc = 100. * test_correct / test_total
            history['test_loss'].append(test_loss)
            history['test_acc'].append(test_acc)
            
            # 현재 epoch에서의 학습 및 평가 결과를 출력합니다.
            print(f'Epoch {epoch+1}/{epochs}: Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.2f}%, Test Loss: {test_loss:.4f}, Test Acc: {test_acc:.2f}%')
        
    except Exception as e:
        # 예상치 못한 예외가 발생한 경우 에러 메시지를 출력하고 None을 반환합니다.
